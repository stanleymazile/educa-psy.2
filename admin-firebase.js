/* ============================================================
   EDUCA-PSY — articles-firebase.js (Version finale optimisée)
   ============================================================ */
...
let ecouteursInitialises = false;
...
function carteArticleHTML(article, featured = false) {
  ...
  const categorie = champLocale(article, "categorie", langue);
  ...
  return `
    <article class="article-card ${featured ? "article-card--featured" : ""} cat-${slug}">
      ${image}
      <span class="tag tag-${slug}">${categorie || article.categorie || ""}</span>
...
function erreurChargementHTML() {
  return `<p class="empty-msg">Impossible de charger les articles pour le moment. Vérifiez la configuration dans firebase-config.js et les règles de sécurité Firestore.</p>`;
}
...
async function initArticlePageFirebase() {
  ...
  zone.innerHTML = `
    ...
    ${imageHero}
    <div class="article-body">${contenu.length ? contenu.map(p => `<p>${formaterTexte(p)}</p>`).join("") : "<p><em>Cet article n'a pas encore de contenu.</em></p>"}</div>
